...
export const adminExportDatabase = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // 1. DDL (Simplified table creation for export)
    // In a real scenario, we'd use pg_dump, but here we'll generate basic CREATE TABLE statements 
    // or just the data. Since the user asked for code to create tables too:
    ...
    for (const table of tables) {
      ...
      // Batch inserts in groups of 100 to avoid huge statements
      const batchSize = 100;
