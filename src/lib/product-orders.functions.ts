...
export const deleteProductOrder = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!isAdmin) throw new Error("Forbidden");
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await supabaseAdmin.from("product_orders").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const updateOrderCustomerData = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) =>
    z
      .object({
        id: z.string().uuid(),
        customer_data: z.record(z.string(), z.string().max(500)),
      })
      .parse(d),
  )
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabaseAdmin
      .from("product_orders")
      .update({ customer_data: data.customer_data as any })
      .eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
