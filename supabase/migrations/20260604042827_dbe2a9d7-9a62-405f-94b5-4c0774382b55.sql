CREATE OR REPLACE FUNCTION public.admin_cron_status()
 RETURNS TABLE(jobid bigint, jobname text, schedule text, active boolean, last_run_started timestamp with time zone, last_run_ended timestamp with time zone, last_status text, last_return_message text, last_http_status integer, last_http_error text)
 LANGUAGE plpgsql
